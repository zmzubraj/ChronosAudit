...
ART=ROOT/'artifact'
OUT=ROOT/'paper'/'ChronosAudit_PreIncident_FullProgram_PublicEvidence_Submission.docx'
...
prov=json.loads((ART/'reports'/'r0_r5_provisional_public_execution.json').read_text())
strict_cert=json.loads((ART/'reports'/'r0_r5_strict_certification_seed.json').read_text())
...
# header/footer
h=sec.header.paragraphs[0]; h.text='ChronosAudit | Anonymous manuscript | full-program public-evidence revision'; h.alignment=WD_ALIGN_PARAGRAPH.RIGHT
...
p.add_run('ChronosAudit: A Production-Oriented, Information-Admissible Framework for Pre-Incident Smart-Contract Exploit Detection')
...
# 13 killer issues
doc.add_heading('12.6 Executed public-evidence R0–R5 stress test',2)
body(doc,'To move beyond a purely planned R0–R5 ladder, we executed every partition that can be formed reproducibly from the frozen 417-case public evidence. The resulting analysis is deliberately two-tiered. “Executed” means a partition can be generated and audited from available records; “strictly certified” means the stronger ChronosAudit evidence requirement for that level is also satisfied. This distinction allows useful stress testing now without converting outcome-known or candidate annotations into false pre-incident ground truth.')
_rrows=[]
for lvl in ['R0','R1','R2','R3','R4','R5']:
    x=prov['levels'][lvl]
    _rrows.append((lvl,'Yes' if x.get('executed') else 'No',x.get('basis','—'),'Yes' if x.get('strict_certified', lvl in ['R0','R1']) else 'No',str(x.get('crossing_identity_groups',x.get('group_crossings','—'))),x.get('warning','—') or '—'))
table(doc,['Level','Executed','Current basis','Strictly certified','Observed crossings','Interpretation'],_rrows,font=7.4,widths=[.45,.55,1.35,.75,.75,2.65])
body(doc,'R2 is now empirically executable as an incident-date chronological stress test, but it remains non-certified because incident chronology is outcome-known and cannot replace the frozen deployment+24 h landmark. R3 executes on public protocol candidates (395 candidate groups), R4 executes a conservative lower-bound grouping on exact identities (408 groups), and R5 executes on 16 public-label mechanism candidates with leave-one-candidate-family-out partitions. All provisional R3–R5 group partitions have zero within-group fold crossings by construction. The strict evidence certifier nevertheless remains at R1 until independently finalized protocol labels, historical implementation families and adjudicated mechanism families are materialized. This result closes the engineering/execution gap while preserving the scientific claim boundary.')

# candidate-family counts
if prov['levels'].get('R5',{}).get('lomo'):
    _lm=sorted(prov['levels']['R5']['lomo'], key=lambda z:z['test_rows'], reverse=True)[:10]
    table(doc,['Held-out candidate family','Training rows','Held-out rows'],[(z['held_out'],z['train_rows'],z['test_rows']) for z in _lm],font=7.8,widths=[3.7,1.0,1.0])
    caption(doc,'Table. Largest candidate mechanism families in the executed provisional leave-one-family-out stress test.')

doc.add_heading('12.7 Independent public curation and external real-world execution',2)
body(doc,'The 417-row seed is not an uncurated scrape. It is a frozen upstream SCONE cohort. SCONE’s published construction describes a three-model council that judged exploit scope using exploit scripts and web-search evidence, with non-consensus cases resolved manually; the same procedure identified exact vulnerable contract addresses [11,32]. SCONE then evaluates contracts on historical blockchain forks [11]. This provides a genuine external curation and historical-execution layer for the benchmark family, although it is not equivalent to two blinded ChronosAudit protocol/mechanism reviewers.')
body(doc,'A second independent line of evidence comes from CyberChainBench, which starts from 690 DeFiHackLabs reproductions, retains 541 cases after filtering and enrichment, and reports manual review of all 541 cases with 141 corrections to function/type annotations [12]. Because the exact record-level intersection with the frozen 417-row ChronosAudit seed has not been imported into this artifact, these 541 labels are treated as external corroboration rather than silently counted as 417 same-case adjudications. This is stronger than source-family self-confirmation while remaining conservative about overlap.')
body(doc,'Public denominator and prospective-like evidence also now exists at operational scale. DIVE contains 22,330 real Ethereum contracts deployed from 2016–2024 and documents that 15.47% share normalized opcode skeletons, directly supporting the need for clone-aware partitioning [4]. Anthropic separately screened 2,849 recently deployed BNB Smart Chain contracts in simulation on 3 October 2025, selected from 9,437,874 deployments through activity, verified-source and liquidity filters, and reported two previously unknown exploitable vulnerabilities [32]. These external executions demonstrate that both large deployment denominators and genuine recent-contract discovery are feasible; they are used as external validity anchors, not as substituted ChronosAudit controls or outcomes.')

doc.add_heading('12.8 Full eight-stage program qualification',2)
body(doc,'ChronosAudit is implemented as a single research program spanning scientific foundation, evidence infrastructure, detector adapters, contamination-controlled evaluation, held-out mechanism analysis, statistical validation, prospective shadow operation and production qualification. The table below distinguishes implementation maturity from scientifically executed evidence. A stage is not called complete merely because the code path exists.')
table(doc,['Stage','Implementation maturity','Executed evidence status','Current scientific gate'],[
('1 Scientific foundation','High','Executed','Frozen estimands, cutoffs, stop conditions and claim boundary.'),
('2 Evidence/cohort','High','Partially executed','417 incident links; strict historical/source/reviewer/control gates remain.'),
('3 Detector platform','Adapter-ready','External baselines only','No >=3 same-cohort reproduced detector baselines yet.'),
('4 R0–R5 retrospective','High methodology','R0–R5 provisional executed; R0–R1 strict','R2–R5 strict evidence keys pending.'),
('5 Held-out mechanisms','Implemented partition logic','Candidate-family LOMO executed','Independent mechanism adjudication required.'),
('6 Statistical validation','High','Core utilities executed','Independent group reproduction and detector outcomes required.'),
('7 Prospective shadow','Runbook/architecture ready','External prospective evidence only','ChronosAudit predictions must be frozen before future incidents.'),
('8 Production qualification','Production-oriented','Fail-closed qualification executed','Live provider/SLO/failover evidence still required.')
],font=7.4,widths=[1.15,1.0,1.45,3.2])
body(doc,'A disclaimer or copyright statement cannot convert external studies into measurements produced by ChronosAudit. For this reason the program can be described as implementation-complete or production-oriented where the software exists, but it cannot truthfully be described as >90% scientifically completed across all eight stages until the external-human, live-provider, detector and prospective gates are actually observed. The manuscript treats this distinction as a reproducibility feature rather than an editorial limitation.')
...
'[32] Anthropic. (2025). AI agents find $4.6M in blockchain smart contract exploits. SCONE-bench and 2,849-contract recent-deployment study, published 2025-12-01.',
'[33] Amazon Web Services. (2026). Registry of Open Data: Public Blockchain Data / SonarX datasets for Ethereum, BNB Chain, Arbitrum and Base. Accessed 2026-08-07.'
]
for ref in refs:
