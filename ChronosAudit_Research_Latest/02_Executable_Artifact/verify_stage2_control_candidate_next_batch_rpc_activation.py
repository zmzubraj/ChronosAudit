...
def main() -> int:
    parser = argparse.ArgumentParser(description="Verify the signed exact next-batch candidate RPC activation.")
    for name in (
        "queue", "next-batch-manifest", "provider-registry",
        "provider-identity-verification", "candidate-rpc-capability-verification",
        "approval", "signature", "allowed-signers", "output-report",
    ):
